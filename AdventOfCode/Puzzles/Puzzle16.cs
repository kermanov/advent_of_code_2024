...
public class Puzzle16 : PuzzleBaseWithProgress
{
    ...
    static Point[] GetAntiNodes(Point antenna1, Point antenna2)
    {
        ...
        var antiNodes1 = new Point[50];
        antiNodes1[0] = antenna2 + delta;
        for (int i = 1; i < 50; i++)
        {
            antiNodes1[i] = antiNodes1[i - 1] + delta;
        }

        var antiNodes2 = new Point[50];
        antiNodes2[0] = antenna2 - delta;
        for (int i = 1; i < 50; i++)
        {
            antiNodes2[i] = antiNodes2[i - 1] - delta;
        }

        return antiNodes1.Concat(antiNodes2).ToArray();
    }
